...
class RNNDataset(Dataset):
    def __init__(self, state_list: list, label_list: list, episode_names: list, batch_size: int, range_params=None):
        """
        ...
        try:
            if not range_params:
                ...
                print('Data is normalized: current experiments indicate that this method results in lower performance. Continue at your own risk.')
        ...
        for state, label, name, unpacked_len in zip(padded_state_list, padded_labels, episode_names, self.state_lens):
            self.episodes.append(
                {'state': torch.tensor(state), 'label': torch.tensor(label), 'name': name, 'length': unpacked_len})
    ...
    def __getitem__(self, index):
        return self.episodes[index]['state'], self.episodes[index]['label'], self.episodes[index]['length'], \
               self.episodes[index]['name']
