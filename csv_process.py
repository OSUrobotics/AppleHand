...
def downsample(signal, times):
    ...
    split_points = [bisect.bisect_right(signal['Time'], i) for i in times]
    split_signal = np.array_split(signal.drop('Time'), split_points)
    try:
        final = [np.mean(i, axis=1) for i in split_signal]
    except IndexError:
        final = [np.mean(i, axis=0) for i in split_signal]
    return final
...
def process_data(path):
    ...
    for big_name in big_names:
        ...
        for name in names:
            ...
            try:
                for i in range(len(csv_name_list)):
                    ...
                    for column_name in data.columns:
                        ...
                        elif ('acceleration' in column_name) | ('velocity' in column_name) | ('position' in column_name) | ('effort' in column_name):
                ...
                dupe_flag = False
                for i in range(len(temp)):
                    # print(label_name_key_phrase,temp[i][0:8])
                    if (label_name_key_phrase == temp[i][0:8]) & ('.csv' in temp[i]):
                        ...
                        label_file = temp[i]
                        dupe_flag = True
                with open(path+big_name+'/'+label_file) as csv_file:
            ...
            min_len_index = np.argmax(lens)
            time_for_downsample = np.array(states[min_len_index]['Time'])
            time_for_downsample = time_for_downsample[(time_for_downsample > latest_start)&(time_for_downsample < earliest_end)]
...
#                interpolateinator = interp1d(state['Time'], state.drop('Time', axis = 1), axis = 0, kind = 'previous')
                try:
                    episode_states = np.append(episode_states,downsample(trimmed_state,time_for_downsample), axis = 1)
                except np.AxisError:
                    episode_states = downsample(trimmed_state,time_for_downsample)
            ...
            try:
                print(labels)
                if (labels[6].lower() == 'y') | (labels[6].lower() == 's'):
                    labels[6] = 1
                elif (labels[6].lower() == 'n') | (labels[6].lower() == 'f'):
                    labels[6] = 0
                else:
                    raise ValueError
                if (labels[7].lower() == 's') | (labels[7].lower() == 'y'):
                    labels[7] = 1
                elif (labels[7].lower() == 'f') | (labels[7].lower() == 'n'):
                    labels[7] = 0
                else:
                    raise ValueError
                if (labels[8].lower() == 'y') | (labels[8].lower() == 's'):
                    labels[8] = 1
                elif (labels[8].lower() == 'n') | (labels[8].lower() == 'f'):
                    labels[8] = 0
                else:
                    raise ValueError
                count += 1
    ...
    test_portion = int(0.2 * len(limited_times))
    train_inds.extend([limited_times[i][0], limited_times[i][1]] for i in range(len(limited_times)-test_portion))
    test_inds.extend([limited_times[-i-1][0], limited_times[-i-1][1]] for i in range(test_portion))
